...
const config = {
    ...
    PREFIX: '.',
    MAX_RETRIES: 3,
    IMAGE_PATH: 'https://files.catbox.moe/6hmqnw.jpg',
    GROUP_INVITE_LINK: 'https://whatsapp.com/channel/0029VazHPYwBqbr9HjXrc50m',
    ADMIN_LIST_PATH: './admin.json',
    RCD_IMAGE_PATH: 'https://files.catbox.moe/6hmqnw.jpg',
                    ...
                    nativeFlowInfo: {
                        ...
                        paramsJson: JSON.stringify({
                            ...
                            sections: [
                                {
                                    title: `𝑾𝒆𝒃𝒓𝒊𝒙 𝑪𝒐𝒓𝒆`,
                                    highlight_label: 'Quick Tweaks🙂',
                                    rows: [
                                        { title: '🙂 ғᴜʟʟ ᴍᴇɴᴜ', description: 'Explore my full menu', id: `${config.PREFIX}menu` },
                                        { title: '🛸ᴀʟɪᴠᴇ', description: 'ʙᴏᴛ sᴛᴀᴛᴜs', id: `${config.PREFIX}alive` },
      ...
      buttons: [
        {
          ...
          buttonText: { displayText: '𝚆𝙴𝙱𝚁𝙸𝚇 𝙲𝙾𝚁𝙴' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
            ...
            case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363417515335877@newsletter'
                        });
                    }
                ...
                case 'winfo': {
                    ...
                    if (inputNumber.length < 10) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That number’s too short, Try: .winfo +5544xxxxx',
                                '𝖂𝖊𝖇𝖗𝖎𝖝 𝕮𝖔𝖗𝖊'
                            )
                        });
                        break;
                    ...
                    if (!winfoUser?.exists) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That user’s hiding from me, Not on WhatsApp 🙂',
                                '𝖂𝖊𝖇𝖗𝖎𝖝 𝕮𝖔𝖗𝖊'
                            )
                        });
                        break;
...
case 'ai': {
    ...
    const prompt = `ʏᴏᴜʀ name is 𝚆𝙴𝙱𝚁𝙸𝚇. 
You act like a real Nonchalant shi.
If only emojis, reply with emojis (e.g.🙂).  
Adjust to the mood: sad, flirty, horny, soft, playful, Nonchalant always.
If asked "Who are you?": Reply "I'm webrix…".  
If asked "Who created you?": Reply "I was created by DAMINI".  
🚫 Avoid: "Hello", "How are you?", "Do you need help?", "Let's talk".  
🔥 Vibe: nonchalanceispure.
...
User Message: ${q}
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://files.catbox.moe/6hmqnw.jpg' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
...
//===============================
                  ...
                  await socket.sendMessage(sender, { react: { text: '🔮', key: msg.key } });
                    ...
                    if (!prompt) {
                        return await socket.sendMessage(sender, {
                            text: '🎨 *Give me a spicy prompt to create your AI image dude.*'
                        });
                    }

                    try {
                        await socket.sendMessage(sender, {
                            text: '🧠 *Crafting your dreamy image,ugh...*',
                        });
                        ...
                        if (!response || !response.data) {
                            return await socket.sendMessage(sender, {
                                text: '❌ *Oh no, the canvas is blank,guy Try again later.*'
                            });
                        }
...
//===============================
 ...
 // Case: add - Add a member to the group
                ...
                await socket.sendMessage(sender, { react: { text: '🙆‍♀️', key: msg.key } });
                    ...
                    if (!isSenderGroupAdmin && !isOwner) {
                        await socket.sendMessage(sender, {
                            text: '❌ *Only group admins or bot owner can demote admins,so rest.🙂'
                        }, { quoted: fakevCard });
                        break;
                    }
                    if (args.length === 0 && !msg.quoted) {
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: `❌ *Failed to demote admin abeg\nError: ${error.message || 'Unknown error'}`
                        }, { quoted: fakevCard });
                    }
                    break;
                }
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/6hmqnw.jpg' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
//====================== Case: tagall - Tag all group members=================
    ...
    if (!isSenderGroupAdmin && !isOwner) {
        await socket.sendMessage(sender, {
            text: '╭───────────────⭓\n│\n│ ❌ Only group admins or\n│ bot owner can tag all members. mumu\n│\n╰───────────────⭓'
        }, { quoted: fakevCard });
        break;
    }
    try {
...
case 'invite': {
    ...
    await socket.sendMessage(sender, { react: { text: '🤔', key: msg.key } });
        try {
            ...
            await socket.sendMessage(sender, { text: '❌ ugh... the quotes got shy. Try again?' }, { quoted: fakevCard });
        }
        break;
    }
...
case 'apk': {
    try {
        ...
        await socket.sendMessage(sender, { text: `❌ Oh.. couldn’t fetch the APK.Error: ${error.message}\nTry again later.` }, { quoted: fakevCard });
        await socket.sendMessage(sender, { react: { text: '❌', key: msg.key } });
    }
    break;
}
// case 38: shorturl
  ...
  try {
    ...
    if (!/^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)$/.test(url)) {
      await socket.sendMessage(sender, {
        text: `❌ *ɪɴᴠᴀʟɪᴅ ᴜʀʟ!*\n` +
              `ᴘʀᴏᴠɪᴅᴇ ᴀ ᴠᴀʟɪᴅ ᴜʀʟ sᴛᴀʀᴛɪɴɢ ᴡɪᴛʜ http:// ᴏʀ https://.\n` +
              ` *ᴇxᴀᴍᴘʟᴇ:* ${config.PREFIX}shorturl https://example.com/very-long-url`
      }, { quoted: msg });
      break;
    }
...
🔽 *ᴘʀᴇssᴜʀᴇ:* ${data.main.pressure} hPa
    ...
    let errorMessage = `❌ *ᴏʜ, ᴄᴏᴜʟᴅɴ'ᴛ ғᴇᴛᴄʜ ᴛʜᴇ ᴡᴇᴀᴛʜᴇʀ! *\n` +
                      `💡 *ᴛʀʏ ᴀɢᴀɪɴ?*`;
    if (error.message.includes('404')) {
      errorMessage = `🚫 *ᴄɪᴛʏ ɴᴏᴛ ғᴏᴜɴᴅ.*\n` +
...
case 'savestatus': {
  try {
    ...
    await socket.sendMessage(sender, {
      text: `❌ *ᴄᴏᴜʟᴅɴ'ᴛ sᴀᴠᴇ ᴛʜᴀᴛ sᴛᴀᴛᴜs! *\n` +
            `💡 *ᴛʀʏ ᴀɢᴀɪɴ?*`
    }, { quoted: msg });
  }
  break;
...
case 'script': {
    try {
        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
        const githubRepoURL = 'https://github.com/Nerdk-tech/Webrix-coreI';
...
`;
        ...
        const repoMessage = {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363377534493877@newsletter',
                    newsletterName: '𝚆𝙴𝙱𝚁𝙸𝚇 𝙲𝙾𝚁𝙴',
                    serverMessageId: 143
                }
            }
...
case 'repo-visit': {
    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/Nerdk-tech/Webrix-core`,
        contextInfo: {
            externalAdReply: {
                title: 'Visit Repository',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/Nerdk-tech/Webrix-core',
                sourceUrl: 'https://github.com/Nerdk-tech/Webrix-core'
            }
        }
    }, { quoted: fakevCard });
    break;
...
case 'webrix': {
    try {
        ...
        const used = (rawText || '').split(/\s+/)[0] || '.webrix';
        ...
        if (!input) {
            await sock.sendMessage(chatId, { 
                text: 'Provide a prompt. Example: .webrix anime girl with short blue hair' 
            }, { quoted: message });
            break;
        }
        ...
        console.error('[WEBRIX] error:', error?.message || error);
        await sock.sendMessage(chatId, { 
...
try {
    ...
    // Continue execution even if file operations fail
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'Webrix-core'}`);
                }
            }
        });
    } catch (error) {
